...
2. **🚫 ZERO POLLUTION / TEMPORARY FILES**: ALL scratch scripts, Puppeteer tests, diagnostic runners, temporary logs, and marker files (`fwd-*`, `done-*`, etc.) **MUST GO STRICTLY INTO `temp/` OR `tmp/`**. It is strictly forbidden to create or leave any temporary, scratch, or test files in the repository root or project source folders. Clean up scripts after execution!
...
1. **Procedural 3D Modeling Blueprint**: Before implementing any new mechanical components, consult `AI_3D_MODELING_GUIDE.md` for geometry primitives, formulas, and builder conventions.
2. **Visual Verification Tool (`npm run inspect:3d`)**: Always run `npm run inspect:3d -- --focus=<component>` to capture headless screenshots and verify that your procedural meshes fit together without unnatural clipping or overlaps.
