...
## 🧪 5. AI Modeler Sandbox (Narzędzie Twórcze)
Aby przyspieszyć i ułatwić tworzenie precyzyjnych kształtów bez ryzyka uszkodzenia logiki w `EngineBuilder` czy `DrivetrainBuilder`, zaimplementowano **AI Modeler Sandbox** (`sandbox.html`).
Jest to wyizolowane środowisko (dostępne z UI pod przyciskiem w DevTools), które zapewnia:
- Pustą przestrzeń z bazową siatką (GridHelper) i osiami (AxesHelper).
- Kontrolki obrotu (OrbitControls) i przeciągania (TransformControls).
- Kontekst do szybkiego prototypowania bloków, kół i przekładni.

### Jak AI powinno tego używać?
Możesz wykonywać kod bezpośrednio na tej scenie, by zwizualizować nowe koncepcje, poprzez dostęp do obiektu w konsoli przeglądarki: `window.AI_MODELER`.
Przykładowo, prosząc AI o stworzenie skomplikowanego sprzęgła hydrokinetycznego, AI może napisać skrypt korzystający z:
```typescript
const m = window.AI_MODELER;
m.clear(); // Czyści scenę
// Tworzenie testowego kształtu (z widocznym wireframe'em do oceny krawędzi)
const obudowa = m.spawnPrimitive('cylinder', { rt: 0.3, rb: 0.3, h: 0.1, s: 32 }, 0xaaaaaa);
const wirnik = m.spawnPrimitive('cylinder', { rt: 0.2, rb: 0.2, h: 0.05, s: 16 }, 0x3b82f6);
// Precyzyjne układanie względem siebie
m.alignTo(wirnik, obudowa, new THREE.Vector3(0, 0.05, 0));
```
Po upewnieniu się, że proporcje, marginesy i nachodzenie (overlapping) obiektów wyglądają prawidłowo, zoptymalizowany, przetestowany kod można bez obaw przenieść do głównego środowiska `DrivetrainBuilder` bądź nowej klasy składowej.
